...
| 16 |  просмотреть содержимое в реальном времени | tail -f log_file && grep -C 3 search_term<br>Пояснение:<br>опция -f (follow) постоянно выводит изменения на экран, т.е. показывает  новые строки, добавленные в файл. Для остановки используется Ctrl +C.<br>соединяем с командой grep для поиска конкретного термина при мониторинге файла журнала, т.к. визуально обнаружить искомое в потоке входящих новых строк практически невозможно.<br>Оопция -С в grep , чтобы показать несколько строк до и после искомого термин, т.к. только строки с искомыми словами не раскрывают необходимых деталей. | 
...
find -type d —  найти папку, а не файл, по имени. То есть использовать find -type d (поиск папки). 
...
start path/to/dir (или текущую ..) - откроет папку
start file.js - откроет файл в редакторе по умолчанию
start 'https://isqua.ru/blog/' - откроет адрес в браузере
